...
func unpackFingerprint(data []byte, fp *Fingerprint) error {
	...
	bits := UnpackUint3Slice(data[offset:])
	numValues := 0
	...
	if numExceptionalBits > 0 {
		exceptionalBits := UnpackUint5Slice(data[offset:])
...
func CompressFingerprint(fp Fingerprint) []byte {
	...
	PackUint3Slice(data[4:], normalBits)
	PackUint5Slice(data[4+normalBitsSize:], exceptionBits)
