...
func matchAlignedFingerprints(master *Fingerprint, query *Fingerprint, offset int) ([]MatchingSection, error) {
	...
	diff := make([]float64, n)
	for i := 0; i < n; i++ {
		diff[i] = float64(PopCount32(masterHashes[i] ^ queryHashes[i]))
	}
	// log.Print(diff)

	smoothedDiff := make([]float64, n)
	GaussianFilter(diff, smoothedDiff, 9, 1.3, Border{Type: BorderReflect})
	// log.Print(smoothedDiff)

	smoothedDiffGradient := make([]float64, n)
	Gradient(smoothedDiff, smoothedDiffGradient)
